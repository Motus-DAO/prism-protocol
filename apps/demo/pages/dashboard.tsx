...
const DevSettingsDashboard = dynamic(
  () => import('../components/prism/DevSettingsDashboard').then(mod => ({ default: mod.DevSettingsDashboard })),
  {
    ssr: false,
    loading: () => (
...
export default function DashboardPage() {
  ...
  return (
    <>
      <Head>
        <title>Dev Settings – Prism Protocol</title>
        <meta name="description" content="Configure Prism: RPC, Program ID, Arcium, context types. Example for developers." />
      ...
      <main className="min-h-screen bg-transparent relative">
        ...
        <nav className="fixed top-0 left-0 right-0 z-40 flex items-center justify-between px-6 py-4 bg-noir/90 backdrop-blur-md border-b border-steel">
          ...
          <div className="flex items-center gap-6">
            ...
            <span className="text-prism-cyan font-landing-mono text-sm uppercase tracking-wider">Dashboard</span>
          </div>
        </nav>
        ...
        <footer className="fixed bottom-4 left-0 right-0 text-center z-30">
          ...
          >
            Prism Protocol v0.1.0 | Dev settings example
          </motion.p>
        </footer>
      </main>
    </>
